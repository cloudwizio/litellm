...
@pytest.mark.asyncio
async def test_metrics_marker_returned_by_ensure_registered():
    """get_metrics_marker returns the metricsMarker value from register response."""
    dest = _dest()
    ...
    # Second call returns None (already registered, cached)
    marker2 = await dest.get_metrics_marker()
    assert marker2 is None
    assert mock_http.client.request.call_count == 1  # no second API call
