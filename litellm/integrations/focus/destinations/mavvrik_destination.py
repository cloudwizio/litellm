...
class FocusMavvrikDestination(FocusDestination):
    ...
    async def get_metrics_marker(self) -> Optional[int]:
        """Register with Mavvrik and return the metricsMarker.

        The metricsMarker is a Unix timestamp (seconds) representing the last
        date Mavvrik has successfully ingested. Used by MavvrikFocusLogger to
        detect and catch up any dates missed due to previous export failures.

        Returns None if already registered this session (marker already consumed).
        """
